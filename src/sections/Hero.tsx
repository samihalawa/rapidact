    ...
    <div className="hairline border bg-white">
      <div className="hairline flex items-baseline justify-between border-b bg-[#16181d] px-6 py-4">
        <div>
          ...
          <p className="mt-1 text-[15px] font-semibold text-white">{REPORT.name}</p>
        </div>
      ...
      <ol className="divide-y divide-[#e2e2dd]">
        {REPORT_CHAPTERS.map((c) => (
          <li key={c.n} className="flex gap-4 px-6 py-3.5">
            <span className="mono ink-soft w-6 shrink-0 pt-0.5 text-[11px]">{c.n}</span>
            <div>
              <p className="ink text-[13px] leading-snug font-semibold">{c.title}</p>
      ...
      <div className="mx-auto grid max-w-6xl gap-14 px-4 pt-16 pb-20 sm:px-6 lg:grid-cols-[1.05fr_1fr] lg:gap-16">
        <div className="lg:pt-4">
          ...
          <div className="hairline flex flex-wrap items-center gap-x-3 gap-y-1 border-l-2 border-l-[#16181d] pl-3">
            <span className="ink text-[13px] font-semibold">EU AI Act, Article 50</span>
          ...
          <p className="ink-soft mt-5 max-w-xl text-[17px] leading-relaxed">
            Most companies do not have a written list of the AI they run, let alone which
            obligations attach to each system. We produce that list for you, classify every system
            against the regulation, and set out what you are required to publish and document.
          </p>

          <p className="ink-soft mt-4 max-w-xl text-[17px] leading-relaxed">
            The assessment costs <span className="ink font-semibold">€99</span>, paid once. It
            reaches your inbox within {REPORT.delivery}. If it does not arrive in that window, you
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Button
              size="lg"
              className="rounded bg-[#16181d] px-7 text-[15px] font-semibold text-white hover:bg-[#2b2f38]"
          ...
          <p className="ink-soft mt-5 max-w-xl text-[13px] leading-relaxed">
            ...
            firm, and the report is not legal advice. Where a question genuinely requires a legal
            opinion, the report says so and tells you what to put in front of counsel.
          </p>
        </div>
