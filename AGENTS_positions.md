...
Shopify marketplace release | a released Shopify CLI version is not a hosted OAuth app or an App Store submission, and review rejects app URLs whose domain contains “shopify” or “example” | host the web service on a neutral branded domain with persistent sessions, release the theme extension, install it on a dev store, then complete the review checks | do not use a provider-named host, call a CLI version public, expose the secret, mutate theme files or claim submission before the dashboard says Submitted | verify the exact release commit, neutral live OAuth URL, active theme embed, rendered storefront badge and App Store review status
Marketplace review locks | Wix can show DECLINED with zero submission blockers while hiding the rejection ticket, and its submission route can fail separately with Error 500 | recover the exact review feedback through the app support channel before changing code, while reproducing WordPress Plugin Check locally if CI cannot start | do not invent a rejection cause, treat zero blockers as approval, or confuse a provider dashboard failure with app-code failure | verify the exact Wix status and reviewer feedback, local Plugin Check JSON, PHP lint, and submitted package bytes
...
RapidAct installation dashboard | marketplace-only UI stranded users before listings were public, while pending-publication labels and stale manifest checks exposed internal release state | show only working installation actions until a real listing URL is verified; offer the WordPress ZIP and retain code alternatives | do not advertise unpublished listings, expose submission status, restore marketplace status to the public badge manifest, or report a copy/download conversion before the user action succeeds | verify ZIP/source parity, all platform/style snippets, bundled runtime parity, clipboard/download feedback, no pending copy, five languages and live phone/desktop layouts
...
## 2026-07-29 — Shopify release requires a neutral host and review proof

- Status: CURRENT
- Project/root: `rapidact`; Shopify app `403881066497`, OAuth service and theme app extension.
- Mistake: treating `shopify app deploy` as hosting or App Store submission, then using `shopify.rapidact.eu` even though Shopify's preliminary review rejects domains containing “Shopify” or “example.”
- Do: host the OAuth service on a neutral RapidAct domain with persistent Prisma storage, release the linked app version, install through OAuth, activate the theme app embed, and complete every preliminary review check.
- Don't: use a provider-named app host, commit or expose the secret, mutate theme files, claim a released version is listed, or attest to an owner agreement without confirmation.
- Evidence: App Store review screenshot on 2026-07-29 showed `Draft`, disabled submit, and the exact failing URL rule; version `1.0.0`, OAuth install count `1`, active embed and rendered storefront badge remain proven.
- Trigger terms: Shopify, OAuth, app URL, domain, public distribution, theme app extension, CLI release, App Store, registration.
- Verify before reuse: neutral domain DNS and TLS, matching Shopify/Coolify app URL, OAuth reinstall, active embed plus rendered badge, zero review blockers, submitted provider state, and local/remote Git SHA.
...
- Mistake: treating `Blockers (0)` or a previously submitted version as proof that Wix review remained healthy, then guessing at code changes without the reviewer ticket.
- Do: read the exact Wix status and rejection feedback first. If feedback is unavailable or `/distribute/submit` fails independently, use the app-development support channel to recover the ticket and restore the provider surface; reproduce WordPress Plugin Check locally when CI cannot start.
- Don't: invent the rejection reason, resubmit blindly, accept the Partner Agreement without owner confirmation, or infer app-code failure from a dashboard 500 or an unstarted GitHub runner.
- Evidence: dashboard showed `DECLINED`, released `1.0`, current profile/listing, `Blockers (0)`, and `/distribute/submit` returned `Error 500`; Wix support accepted the request “Declined app feedback unavailable and submission page returns Error 500” for email response within 72 hours.
- Trigger terms: declined, rejected, Blockers 0, rejection ticket, submit page, Error 500, app not editable, plugin check.
- Verify before reuse: obtain the exact reviewer feedback, repair that stated defect, re-run Wix app checks, then read the resubmitted dashboard state; separately run PHP lint, local Plugin Check, and byte-compare the WordPress ZIP.
