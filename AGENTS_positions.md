...
Shopify marketplace release | a released CLI version is not a hosted/submitted app, provider-named domains fail review, and an authenticated document loader alone does not generate observable session-token telemetry | use a neutral host, AppProvider, persistent sessions and one client request to an authenticate.admin action, then exercise the installed dev-store app | do not rely on initial SSR authentication, use a provider-named host, expose the secret, mutate theme files or claim submission before the dashboard says Submitted | verify exact release commit, live OAuth URL, active embed, storefront badge, both embedded checks green and submitted provider state
...
- Mistake: treating `shopify app deploy` as hosting/submission, using a provider-named domain, and assuming an authenticated SSR loader would satisfy Shopify's session-token telemetry without a client request after App Bridge initialized.
...
- Verify before reuse: neutral DNS/TLS, matching app URL, loaded embedded app, authenticated client action, active embed plus storefront badge, both embedded checks green, submitted provider state, and local/remote Git SHA.
