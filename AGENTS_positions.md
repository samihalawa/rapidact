...
RapidAct installation dashboard | pre-publication fallbacks can remain after a marketplace app becomes the simpler verified path | make the published Wix app the only Wix route, keep WordPress ZIP and code options where they still solve real access gaps, and write store copy for buyers rather than reviewers | do not show Wix code/display alternatives, internal implementation language, duplicate headings or release-status copy | verify the Wix add-to-site destination, one-path modal, listing copy/keywords/media, other platform snippets, five languages and live phone/desktop layouts
...
- Do: lead with the official app, show three buyer steps plus the actual visitor result, and use EU AI Act, Article 50, AI transparency and AI disclosure language in the listing.
...
- Evidence: user correction and public App Market listing on 2026-07-30; `src/components/BadgeInstallDashboard.tsx`, `src/data/{badgeInstaller,platforms}.ts`.
- Trigger terms: Wix, install options, App Market, ASO, keywords, visual explanation, technical copy.
- Verify before reuse: saved/reloaded listing fields and keywords, public listing media, `https://wix.to/JKi80ih`, absence of Wix manual controls, five locales, and phone/desktop rendering.
