...
RapidAct initial JavaScript | eager route imports and lightweight language constants coupled to the eager Markdown registry load every route and article on the homepage | lazy-load client routes, keep SSR eager for prerendering, and isolate shared language metadata from content data | do not put TRPC providers or content registries above every route | verify prerendered HTML, hydration on static and TRPC routes, bundle gzip, SEO verifier and a fresh production Lighthouse run
...
RapidAct production asset continuity | a cached HTML shell can reference a removed hashed bundle, while a fixed-path badge runtime can remain stale after a successful deploy | revalidate HTML and mutable public runtimes, version the site's runtime request, and cache only versioned/hashed assets immutably | do not diagnose the bundle from the badge alone, trust a deployment without reading the public body, or apply immutable caching to a fixed path | verify fresh HTML/runtime bodies and headers, referenced assets, and rendered routes after consecutive deploys
...
- Do: lazy-load page modules in the client, resolve them eagerly only in the SSR build, keep tRPC providers on the three routes that use them, and store language constants in a dependency-free module.
- Don't: weaken prerendering, duplicate route surfaces, or import the eager content registry into global providers/navigation.
...
- Mistake: a previously loaded HTML shell still referenced a removed hashed JavaScript bundle after deployment; later, Cloudflare retained the fixed `/rapidact-badge.js` for four hours and served old content after the replacement deploy.
- Do: revalidate HTML and mutable fixed-path runtimes, version the site's runtime request, send `no-store` on missing assets, and cache only successful content-hashed or explicitly versioned assets immutably.
- Don't: infer the app bundle is healthy from the badge, close on a cache-busted probe alone, or apply immutable/default edge caching to a mutable fixed path.
- Evidence: 2026-07-30 blank-page bundle incident; on 2026-08-03 the deployed badge path returned `cf-cache-status: HIT`, `max-age=14400` and old `More information`, while the versioned request returned `Read the Article 50 guide`.
- Trigger terms: blank page, white page, badge only, deploy, hashed asset, stale HTML, 404 bundle, cache.
- Verify before reuse: HTML and fixed runtime responses have their intended revalidation policy, their bodies match the release, named JS/CSS return 200, and the exact production route renders after a subsequent deploy.
