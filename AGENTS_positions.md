...
RapidAct scanner engine | a four-step one-page scan can still fail on long editorial pages when the prompt encourages top-to-bottom reading | use native `gemini-computer-use` for one bounded six-step Anchor task that inspects the initial viewport, allows at most one confirming scroll, and reserves the final step for structured output | do not read article copy top-to-bottom, synthesize a zero-result from `max steps reached`, auto-retry, simulate percentage progress, or treat AI-related content and badges as functional AI | verify deepswapai.io returns structured findings, rapidact.eu/article-50 returns a structured zero-result, the exact step-limit sentinel still fails in tests, and production serves the pushed assets
...
## 2026-08-04 — The free Anchor scan must reserve enough steps to return structured output

- Status: CURRENT
- Project/root: `rapidact`; `/scanner`, `api/routers/scan.ts`, Anchor Browser `perform-web-task`.
- Mistake: the one-page fast scan was correctly narrowed, but the four-step budget plus “read the visible page from top to bottom” prompt still exhausted the task on long editorial pages and returned the plain provider sentinel `Task completed (max steps reached)` instead of structured JSON.
- Do: keep `gemini-computer-use` on one submitted rendered page, but bound it as an initial-viewport-first scan with at most one confirming scroll, reserve the last step for the structured response, accept a structured empty `ai_touchpoints` array as a valid completion, and keep the ceiling modest at six steps.
- Don't: navigate, click, submit, perform Article 5/Annex III review, read article copy top-to-bottom, auto-retry, convert the step-limit sentinel into a zero-result, expose provider text publicly, or show a simulated completion percentage.
- Evidence: workflow `86772` completed the exact production request in two steps with one visited URL and one directly evidenced face-swap touchpoint; workflows `86817` and `86819` completed with raw result `Task completed (max steps reached)` after four steps on `rapidact.eu/article-50`; the public production tRPC flow for workflow `86859` ended with `{"status":"failed","error":"anchor-invalid-result"}` before this repair.
- Trigger terms: scanner speed, less complete, one page, Anchor Browser, prompt, max steps reached, anchor-invalid-result, zero-result, fallback.
- Verify before reuse: `deepswapai.io` returns structured findings with one inspected URL and at most five deduplicated touchpoints; `rapidact.eu/article-50` returns a structured zero-result instead of fallback; the exact step-limit sentinel still fails in unit tests; PDF plus EN/ES/DE/FR/IT phone/desktop UI remain intact; production serves the pushed commit/assets.
