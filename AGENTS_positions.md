...
RapidAct installation dashboard | marketplace-only UI stranded users before listings were public, while pending-publication labels and stale manifest checks exposed internal release state | show only working direct platform installation until a real listing URL is verified; retain native packages internally and verify their versions from local package metadata | do not advertise unpublished listings, expose submission status, restore marketplace status to the public badge manifest, publish direct ZIPs, or report a copy conversion before the clipboard succeeds | verify all platform/style snippets, bundled runtime parity, local package versions, clipboard feedback, preview, no pending copy, five languages and live phone/desktop layouts
